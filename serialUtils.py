...
def addSensorSerial(port, baudrate=9600):
    ...
    if not exists:
        try:
            sensorSerial = serial.Serial(port=port, baudrate=baudrate, timeout=0.1)

            sensorSerial.reset_input_buffer()
            sensorSerials.append(sensorSerial)
            created = True
            print(
                "[INFO] Connected to port: ", port, " baudrate: ", sensorSerial.baudrate
            )
    ...
    while True:
        ...
        for sensorSerial in sensorSerials:
            ...
            while sensorSerial.in_waiting > 0:
                if sensorSerial.in_waiting > 400:
                    sensorSerial.reset_input_buffer()
                    continue
                try:
